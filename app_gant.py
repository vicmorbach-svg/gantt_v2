...
def process_uploaded_report(df_report_raw):
    ...
    # Assumindo a estrutura: Nome do agente, Dia, Hora início, Hora término, Estado, Duração
    df = df_report_raw.copy() # Trabalha com uma cópia para evitar SettingWithCopyWarning

    # Renomear colunas para facilitar o acesso
    # Mapeamento baseado no arquivo Tempo_em_Status_do_agente_por_dia_03202026_1006.xlsx
    df.rename(columns={
        df.columns[0]: 'Nome do agente',
        df.columns[1]: 'Dia', # Este é o "Dia do mês"
        df.columns[2]: 'Hora de início do estado - Carimbo de data/hora',
        df.columns[3]: 'Hora de término do estado - Carimbo de data/hora',
        df.columns[4]: 'Estado',
        df.columns[5]: 'Duração' # Este é o "Tempo do agente no estado / Minutos"
    }, inplace=True)
...
def process_uploaded_scale(df_scale_raw):
    df = df_scale_raw.copy() # Trabalha com uma cópia

    # Renomear colunas para facilitar o acesso e normalizar
    # Mapeamento baseado no arquivo escala_gantt.xlsx
    df.rename(columns={'NOME': 'Nome do agente',
                       'DIAS DE ATENDIMENTO': 'Dias da Semana',
                       'ENTRADA': 'Entrada',
                       'SAÍDA': 'Saída'}, inplace=True)
    ...
    # Aplicar a função to_time para as colunas de Entrada e Saída
    df['Entrada'] = df['Entrada'].apply(to_time)
    df['Saída'] = df['Saída'].apply(to_time)

    # Remover linhas onde Entrada ou Saída resultaram em None (conversão falhou)
    ...
    for _, row in df.iterrows():
        ...
        # Lidar com a coluna 'Dias da Semana' que pode ter 'Seg, Ter, Qua' ou 'Seg e Qui loja, Ter, Qua e Sex Call'
        dias_atendimento_str = str(row['Dias da Semana']).upper().replace(' ', '').replace('E', ',').split(',')
        entrada = row['Entrada']
        saida = row['Saída']
        carga = row.get('CARGA') # Usar .get() para evitar KeyError se a coluna não existir

        for dia_str in dias_atendimento_str:
            dia_str_clean = unicodedata.normalize('NFKD', dia_str).encode('ascii', 'ignore').decode('utf-8').strip()
            if dia_str_clean in dias_map:
                ...
                pass # Silenciar warnings para 'CALL' e 'LOJA' ou outros não mapeados
...
def calculate_metrics(df_real_status, df_escala, selected_agents, start_date, end_date):
    ...
    for agent in selected_agents:
        ...
        while current_date_metrics <= end_date:
            ...
            daily_scale_entries = agent_df_escala[agent_df_escala['Dia da Semana Num'] == day_of_week_num]

            if not daily_scale_entries.empty:
                for _, scale_entry in daily_scale_entries.iterrows():
                    ...
                    if pd.notna(scale_start_time) and pd.notna(scale_end_time):
                        ...
                        # Ajustar para escalas que viram o dia (ex: 22:00 - 06:00)
                        ...
                        # Filtrar status real para o dia atual (e potencialmente o dia seguinte se o status atravessar a meia-noite)
                        ...
                        for _, status_entry in daily_real_status.iterrows():
                            if status_entry['Estado'] == 'Unified online':
                                ...
                                # Interseção entre o período da escala e o período online
...
with tab_visualization:
    ...
    if 'df_real_status' not in st.session_state or 'df_escala' not in st.session_state:
